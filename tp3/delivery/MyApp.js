...
class MyApp  {
    ...
    initCameras() {
        ...
        let carCam = new THREE.PerspectiveCamera(75, aspect, 0.1, 100000);
        carCam.position.set(0, 400, -750);
        carCam.name = "CarCam";
        carCam.lookAt(new THREE.Vector3(1000, 0, -1000))
        this.cameras["CarCam"] = carCam;
    }
    ...
    */
    ...
    render () {
        this.stats.begin()
        if(this.gui.isDestroyed && !this.startGame && !this.gameRunning){
            this.contents.actuallyLoadContents()
            this.startGame = true
        }
        if(this.startGame && this.gui.countdownOver){
            this.gameRunning = true;
            this.clock = new THREE.Clock();
            this.contents.eLapTimer = this.clock.getElapsedTime();
            this.currentLap = 1;
            this.enemyLap = 1;
            this.startGame = false;
        }
        ...
        if (this.activeCamera !== undefined && this.activeCamera !== null && this.gameRunning && !this.gameOver) {
        ...
        if(player){
            this.cameras["CarCam"].lookAt(player.position)
        }

        if(this.currentLap > 3 || this.enemyLap > 3){
            this.gameOver = true;
            this.setActiveCamera("Top");
            this.gui.gameOver();
        }
